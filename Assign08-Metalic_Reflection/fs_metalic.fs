...
vec3 getXYZ2(float cos_theta, vec3 col){
	vec3 color = vec3(0.0,0.0,0.0);
	float maxY = 0.0;
	
	for(int i = 0; i < SPECTRUM_LENGTH; i++){
		float f = fresnel(cos_theta, nf[i], kf[i]);
		color.x += cie_colour_match[i*2].x * f * col.r;
		color.y += cie_colour_match[i*2].y * f * col.g;
		color.z += cie_colour_match[i*2].z * f * col.b;
...
vec3 getXYZ(float cos_theta, float intensity){
	vec3 color = vec3(0.0,0.0,0.0);
	float maxY = 0.0;
...
void main(void){
	...
	vec3 xyz_color = getXYZ(cos_theta_i, env_intensity);
	//vec3 xyz_color = getXYZ2(cos_theta_i, env_color);
